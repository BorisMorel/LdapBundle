...
{
    private $ldapConnection, $username, $password, $_ldapUser;
    ...
    {
        return (bool) $this->setUsername($username)
                ->addLdapUser();
    }

    public function auth()
    {
        return (bool) $this->doPass()
                ->bind();
    }
    ...
    private function addLdapRoles()
    {
        if (!$this->_ldapUser)
            throw new \Exception(
                    'AddRoles() can be involved only when addUser() have return an user');
    ...
    private function getUserId()
    {
        switch ($this->params['role']['user_id']) {
            ...
            default:
                throw new \Exception(
                        sprintf(
                                'The value can\'t be retrieve for this user_id : %s',
